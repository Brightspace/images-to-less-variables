	...
	vfs.src(src, { base: './' }).pipe(
		...
		through2.obj(function(file, enc, done) {
			...
			var writeVariable = function(tempFile) {
				...
				var value;
				if (extension === '.png') {
					var base64EncodedPng = tempFile.contents.toString('base64');
					value = formatter.formatPng(base64EncodedPng);
				} else if (extension === '.svg') {
					var uriEncodedSvg = encodeURIComponent(tempFile.contents.toString('utf8'));
					value = formatter.formatSvg(uriEncodedSvg);
				} else {
					deferred.reject(new Error('Unsupported file type.'));
					return;
				}
				...
				if (variableStream) {
					variableStream.write(formatter.formatVariable(variableName, value) + '\n');
				}
